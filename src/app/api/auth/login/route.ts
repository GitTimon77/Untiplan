...
export async function POST(request: Request) {
  try {
    ...
    const auth = await verifyLogin(input);
    ...
    if (error instanceof z.ZodError) {
      return errorResponse(error, "Bitte alle Felder ausfüllen.", 400);
    }
    return errorResponse(error, "Anmeldung bei WebUntis fehlgeschlagen. Probiere es erneut.", 401);
  }
}
