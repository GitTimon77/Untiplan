...
import type { Course, Lesson, MessageOfDay, MessagesOfDayPayload, TimetableElement, TimetableElementSelection, TimetableElementType, TimetablePayload, UntisMessage, UntisMessagesPayload } from "@/lib/types";
...
export function Dashboard({displayName,filterStorageId,initialWeek,previewData,previewMessages=emptyMessages,previewUntisMessages=emptyUntisMessages,defaultElement}:{displayName:string;filterStorageId:string;initialWeek?:string;previewData?:TimetablePayload;previewMessages?:MessageOfDay[];previewUntisMessages?:UntisMessage[];defaultElement?:TimetableElementSelection|null}) {
 ...
 useEffect(()=>{if(previewData)return;const refresh=()=>{if(document.visibilityState==="visible"&&navigator.onLine){load();if(mode==="today")loadMessages();if(mode==="messages")loadInbox()}};const timer=window.setInterval(refresh,300000);document.addEventListener("visibilitychange",refresh);return()=>{window.clearInterval(timer);document.removeEventListener("visibilitychange",refresh)}},[load,loadInbox,loadMessages,mode,previewData]);
 ...
 function refreshDashboard(){if(mode==="messages"){void loadInbox();return}void load();if(mode==="today")void loadMessages()}
 ...
 return <div className="app-layout"><main className="app-shell">
  <header className="topbar"><a className="brand" href="/stundenplan"><Image className="brand-icon" src="/untiplan-logo.png" alt="" width={36} height={36} priority/><b>Untiplan</b></a><div className="user"><span>{displayName}</span><button className="ghost" onClick={openAccounts} aria-haspopup="dialog">Konten</button><button className="ghost" onClick={openLogout} aria-haspopup="dialog">Konto entfernen</button></div></header>
   <section className="toolbar"><div>{mode==="messages"?<><p className="eyebrow">Mitteilungen</p><h1>Posteingang</h1><p className="muted">Mitteilungen aus WebUntis</p></>:<><p className="eyebrow">Stundenplan</p><h1>{week.toLocaleDateString("de-DE",{day:"2-digit",month:"long"})} – {addDays(week,4).toLocaleDateString("de-DE",{day:"2-digit",month:"long",year:"numeric"})}</h1>{data?.schoolYear&&<p className="muted">Schuljahr {data.schoolYear}{data.latestImportTime?` · WebUntis-Stand ${new Date(data.latestImportTime).toLocaleString("de-DE")}`:""}</p>}</>}<p className="sync-status" role="status"><span className={isOnline?"online":"offline"}>{isOnline?"Online":"Offline"}</span>{mode!=="messages"&&lastUpdated?<span>Aktualisiert {relativeTime(lastUpdated,clock)}</span>:null}{isOfflineData&&mode!=="messages"?<b>Gespeicherter Stand</b>:null}</p></div><div className="toolbar-actions">{mode!=="today"&&mode!=="messages"&&<><button className="ghost" onClick={()=>setWeek(addDays(week,-7))} aria-label="Vorherige Woche">←</button><button className="ghost" onClick={showCurrentWeek}>Aktuelle Woche</button><button className="ghost" onClick={()=>setWeek(addDays(week,7))} aria-label="Nächste Woche">→</button></>}<button className="ghost refresh-button" onClick={refreshDashboard} disabled={mode==="messages"?inboxBusy:busy||messagesBusy} aria-label={mode==="messages"?"Mitteilungen aktualisieren":"Stundenplan aktualisieren"}>{(mode==="messages"?inboxBusy:busy||messagesBusy)?"…":"↻"}</button>{mode!=="messages"&&<button className="filter-button" onClick={()=>setFiltersOpen(true)} aria-haspopup="dialog">Optionen{courseFilter.filterEnabled?` (${courseFilter.selectedCourseKeys.length})`:""} {courseFilter.filterEnabled?<span className="dot" aria-hidden="true"/>:null}</button>}</div></section>
