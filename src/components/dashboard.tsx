...
import type { Course, Homework, HomeworksPayload, Lesson, MessageOfDay, MessagesOfDayPayload, TimetableElement, TimetableElementSelection, TimetableElementType, TimetablePayload, UntisMessage, UntisMessagesPayload } from "@/lib/types";
import { addDays, fromUntisDate, isoDate, mondayFor } from "@/lib/date";
...
import { HomeworksView } from "@/components/homeworks-view";
import { applyHomeworkCourseFilter } from "@/lib/homeworks";

const timetableElementLabels:Record<TimetableElementType,string> = {1:"Klassen",2:"Lehrkräfte",3:"Fächer",4:"Räume",5:"Schüler*innen"};
const emptyMessages:MessageOfDay[]=[];
const emptyUntisMessages:UntisMessage[]=[];
const emptyHomeworks:Homework[]=[];
...
export function Dashboard({displayName,filterStorageId,initialWeek,previewData,previewMessages=emptyMessages,previewUntisMessages=emptyUntisMessages,previewHomeworks=emptyHomeworks,defaultElement}:{displayName:string;filterStorageId:string;initialWeek?:string;previewData?:TimetablePayload;previewMessages?:MessageOfDay[];previewUntisMessages?:UntisMessage[];previewHomeworks?:Homework[];defaultElement?:TimetableElementSelection|null}) {
 ...
 const [messages,setMessages]=useState<MessageOfDay[]>(previewMessages);
 const [messagesSourceUrl,setMessagesSourceUrl]=useState("");
 const [messagesBusy,setMessagesBusy]=useState(false);
 const [messagesError,setMessagesError]=useState("");
 const [inboxMessages,setInboxMessages]=useState<UntisMessage[]>(previewUntisMessages);
 const [inboxSourceUrl,setInboxSourceUrl]=useState("");
 const [inboxBusy,setInboxBusy]=useState(false);
 const [inboxError,setInboxError]=useState("");
 const [homeworks,setHomeworks]=useState<Homework[]>(previewHomeworks);
 const [homeworksSourceUrl,setHomeworksSourceUrl]=useState("");
 const [homeworksBusy,setHomeworksBusy]=useState(false);
 ...
 const homeworksRequestId=useRef(0);
 ...
 const messagesDate=useMemo(()=>previewData?addDays(week,day):clock?new Date(clock):null,[previewData,week,day,clock]);
 const messagesDateKey=messagesDate?isoDate(messagesDate):"";
 ...
 const loadMessages=useCallback(async(signal?:AbortSignal)=>{
  if(previewData){setMessages(previewMessages);setMessagesError("");setMessagesBusy(false);return}
  if(!messagesDateKey)return;
  const requestId=++messagesRequestId.current;
  const isStale=()=>signal?.aborted||requestId!==messagesRequestId.current;
  setMessagesBusy(true);setMessagesError("");
  try{
   const response=await fetch(`/api/news?date=${messagesDateKey}`,{cache:"no-store",signal});
   const body=await response.json() as MessagesOfDayPayload&{error?:string};
   if(isStale())return;
   if(response.status===401){router.replace("/login");return}
   if(!response.ok)throw new Error(body.error||"Nachrichten zum Tag konnten nicht geladen werden.");
   setMessages(body.messages||[]);setMessagesSourceUrl(body.sourceUrl||"");
  }catch(requestError){
   if(isStale())return;
   setMessagesError(requestError instanceof Error?requestError.message:"Nachrichten zum Tag konnten nicht geladen werden.");
  }finally{if(!isStale())setMessagesBusy(false)}
 },[messagesDateKey,previewData,previewMessages,router]);
 const loadInbox=useCallback(async(signal?:AbortSignal)=>{
  if(previewData){setInboxMessages(previewUntisMessages);setInboxError("");setInboxBusy(false);return}
  const requestId=++inboxRequestId.current;
  const isStale=()=>signal?.aborted||requestId!==inboxRequestId.current;
  setInboxBusy(true);setInboxError("");
  try{
   const response=await fetch("/api/messages",{cache:"no-store",signal});
   const body=await response.json() as UntisMessagesPayload&{error?:string};
   if(isStale())return;
   if(response.status===401){router.replace("/login");return}
   if(!response.ok)throw new Error(body.error||"Mitteilungen konnten nicht geladen werden.");
   setInboxMessages(body.messages||[]);setInboxSourceUrl(body.sourceUrl||"");
  }catch(requestError){
   if(isStale())return;
   setInboxError(requestError instanceof Error?requestError.message:"Mitteilungen konnten nicht geladen werden.");
  }finally{if(!isStale())setInboxBusy(false)}
 },[previewData,previewUntisMessages,router]);
 const loadHomeworks=useCallback(async(signal?:AbortSignal)=>{
  if(previewData){setHomeworks(previewHomeworks);setHomeworksError("");setHomeworksBusy(false);return}
  const requestId=++homeworksRequestId.current;
  const isStale=()=>signal?.aborted||requestId!==homeworksRequestId.current;
  const anchor=new Date();
  setHomeworksBusy(true);setHomeworksError("");
  try{
   const response=await fetch(`/api/homeworks?start=${isoDate(addDays(anchor,-31))}&end=${isoDate(addDays(anchor,120))}`,{cache:"no-store",signal});
   const body=await response.json() as HomeworksPayload&{error?:string};
   if(isStale())return;
   if(response.status===401){router.replace("/login");return}
   if(!response.ok)throw new Error(body.error||"Hausaufgaben konnten nicht geladen werden.");
   setHomeworks(body.homeworks||[]);setHomeworksSourceUrl(body.sourceUrl||"");
  }catch(requestError){
   if(isStale())return;
   setHomeworksError(requestError instanceof Error?requestError.message:"Hausaufgaben konnten nicht geladen werden.");
  }finally{if(!isStale())setHomeworksBusy(false)}
 },[previewData,previewHomeworks,router]);
 ...
 useEffect(()=>{if(mode!=="today")return;const controller=new AbortController();const initial=window.setTimeout(()=>loadMessages(controller.signal),0);return()=>{window.clearTimeout(initial);controller.abort()}},[loadMessages,mode]);
 useEffect(()=>{if(mode!=="messages")return;const controller=new AbortController();const initial=window.setTimeout(()=>loadInbox(controller.signal),0);return()=>{window.clearTimeout(initial);controller.abort()}},[loadInbox,mode]);
 useEffect(()=>{if(mode!=="homeworks")return;const controller=new AbortController();const initial=window.setTimeout(()=>loadHomeworks(controller.signal),0);return()=>{window.clearTimeout(initial);controller.abort()}},[loadHomeworks,mode]);
 ...
 useEffect(()=>{if(previewData)return;const refresh=()=>{if(document.visibilityState==="visible"&&navigator.onLine){if(mode==="homeworks")loadHomeworks();else if(mode==="messages")loadInbox();else{load();if(mode==="today")loadMessages()}}};const timer=window.setInterval(refresh,300000);document.addEventListener("visibilitychange",refresh);return()=>{window.clearInterval(timer);document.removeEventListener("visibilitychange",refresh)}},[load,loadHomeworks,loadInbox,loadMessages,mode,previewData]);
 ...
 const filteredHomeworks=useMemo(()=>applyHomeworkCourseFilter(homeworks,data?.courses||[],courseFilter.selectedCourseKeys,courseFilter.filterEnabled),[courseFilter,data?.courses,homeworks]);
 ...
 const todayTimetableMessage=displayBlocked?"Die Schule hat den Stundenplan für diesen Zeitraum in WebUntis nicht freigegeben.":error&&!data?error:!previewData&&!selectedElement?"Kein Stundenplan ausgewählt.":"";
 ...
 function refreshDashboard(){if(mode==="homeworks"){void loadHomeworks();return}if(mode==="messages"){void loadInbox();return}void load();if(mode==="today")void loadMessages()}
 async function exportImage(){
  try{
   const exportDates=mode==="week"?weekDays.map((_,index)=>dateNumber(addDays(week,index))):[dateNumber(mode==="today"?overviewDate:addDays(week,day))];
   const exportLessons=filteredLessons.filter(lesson=>exportDates.includes(lesson.date));
   const exportStart=mode==="week"?week:mode==="today"?overviewDate:addDays(week,day);
   const title=mode==="week"?`Untiplan · ${week.toLocaleDateString("de-DE",{day:"2-digit",month:"short"})} – ${addDays(week,4).toLocaleDateString("de-DE",{day:"2-digit",month:"short",year:"numeric"})}`:`Untiplan · ${exportStart.toLocaleDateString("de-DE",{weekday:"long",day:"2-digit",month:"long",year:"numeric"})}`;
   downloadBlob(`untiplan-${mode==="week"?`woche-${isoDate(week)}`:isoDate(exportStart)}.png`,await timetablePng(exportLessons,title,{dates:exportDates,bounds:timelineBounds}));
   setActionMessage(mode==="week"?"Wochenplanbild wurde erstellt.":"Tagesplanbild wurde erstellt.");
  }catch(exportError){setError(exportError instanceof Error?exportError.message:"Bild konnte nicht erstellt werden.")}
 }
 ...
 return <div className="app-layout"><main className="app-shell">
  <header className="topbar"><a className="brand" href="/stundenplan"><Image className="brand-icon" src="/untiplan-logo.png" alt="" width={36} height={36} priority/><b>Untiplan</b></a><div className="user"><span>{displayName}</span><button className="ghost" onClick={openAccounts} aria-haspopup="dialog">Konten</button><button className="ghost" onClick={openLogout} aria-haspopup="dialog">Konto entfernen</button></div></header>
   <section className="toolbar"><div>{mode==="messages"?<><p className="eyebrow">Mitteilungen</p><h1>Posteingang</h1><p className="muted">Mitteilungen aus WebUntis</p></>:mode==="homeworks"?<><p className="eyebrow">Aufgaben</p><h1>Hausaufgaben</h1><p className="muted">Deine Aufgaben aus WebUntis</p></>:<><p className="eyebrow">Stundenplan</p><h1>{week.toLocaleDateString("de-DE",{day:"2-digit",month:"long"})} – {addDays(week,4).toLocaleDateString("de-DE",{day:"2-digit",month:"long",year:"numeric"})}</h1>{data?.schoolYear&&<p className="muted">Schuljahr {data.schoolYear}{data.latestImportTime?` · WebUntis-Stand ${new Date(data.latestImportTime).toLocaleString("de-DE")}`:""}</p>}</>}<p className="sync-status" role="status"><span className={isOnline?"online":"offline"}>{isOnline?"Online":"Offline"}</span>{mode!=="messages"&&mode!=="homeworks"&&lastUpdated?<span>Aktualisiert {relativeTime(lastUpdated,clock)}</span>:null}{isOfflineData&&mode!=="messages"&&mode!=="homeworks"?<b>Gespeicherter Stand</b>:null}</p></div><div className="toolbar-actions">{(mode==="week"||mode==="day")&&<><button className="ghost" onClick={()=>setWeek(addDays(week,-7))} aria-label="Vorherige Woche">←</button><button className="ghost" onClick={showCurrentWeek}>Aktuelle Woche</button><button className="ghost" onClick={()=>setWeek(addDays(week,7))} aria-label="Nächste Woche">→</button></>}<button className="ghost refresh-button" onClick={refreshDashboard} disabled={mode==="messages"?inboxBusy:mode==="homeworks"?homeworksBusy:busy||messagesBusy} aria-label={mode==="messages"?"Mitteilungen aktualisieren":mode==="homeworks"?"Hausaufgaben aktualisieren":"Stundenplan aktualisieren"}>{(mode==="messages"?inboxBusy:mode==="homeworks"?homeworksBusy:busy||messagesBusy)?"…":"↻"}</button>{mode!=="messages"&&<button className="filter-button" onClick={()=>setFiltersOpen(true)} aria-haspopup="dialog">Optionen{courseFilter.filterEnabled?` (${courseFilter.selectedCourseKeys.length})`:""} {courseFilter.filterEnabled?<span className="dot" aria-hidden="true"/>:null}</button>}</div></section>
   <nav className="view-tabs" role="tablist" aria-label="Hauptansicht"><button role="tab" aria-selected={mode==="today"} className={mode==="today"?"active":""} onClick={showToday}>Heute</button><button role="tab" aria-selected={mode==="week"} className={mode==="week"?"active":""} onClick={()=>setMode("week")}>Woche</button><button role="tab" aria-selected={mode==="day"} className={mode==="day"?"active":""} onClick={()=>setMode("day")}>Tag</button><button role="tab" aria-selected={mode==="homeworks"} className={mode==="homeworks"?"active":""} onClick={()=>setMode("homeworks")}>Hausaufgaben{filteredHomeworks.some(homework=>!homework.completed)?<span className="tab-badge" aria-label={`${filteredHomeworks.filter(homework=>!homework.completed).length} offen`}>{filteredHomeworks.filter(homework=>!homework.completed).length}</span>:null}</button><button role="tab" aria-selected={mode==="messages"} className={mode==="messages"?"active":""} onClick={()=>setMode("messages")}>Mitteilungen{inboxMessages.some(message=>!message.isRead)?<span className="tab-badge" aria-label={`${inboxMessages.filter(message=>!message.isRead).length} ungelesen`}>{inboxMessages.filter(message=>!message.isRead).length}</span>:null}</button></nav>
   {mode!=="messages"&&mode!=="homeworks"&&<><div className="export-bar"><details className="export-menu"><summary>Ausgeben</summary><div role="menu" aria-label="Stundenplan ausgeben"><button role="menuitem" onClick={event=>{event.currentTarget.closest("details")?.removeAttribute("open");void exportImage()}} disabled={!filteredLessons.length}>Als Bild speichern</button><button role="menuitem" onClick={event=>{event.currentTarget.closest("details")?.removeAttribute("open");window.print()}}>PDF/Drucken</button></div></details></div>{actionMessage&&<p className="action-message" role="status">{actionMessage}</p>}</>}
 {(mode==="today"||mode==="week"||mode==="day")&&!previewData&&!elementsBusy&&timetableElements.length>0&&<div className="plan-picker" aria-label="Stundenplan auswählen"><label><span>Typ</span><select value={selectedElementType} onChange={event=>{const type=Number(event.target.value) as TimetableElementType;const first=timetableElements.find(element=>element.type===type);if(first){setSelectedElement({type:first.type,id:first.id});setSelected(null)}}}>{([1,2,3,4,5] as TimetableElementType[]).filter(type=>timetableElements.some(element=>element.type===type)).map(type=><option key={type} value={type}>{timetableElementLabels[type]}</option>)}</select></label><label className="plan-element-select"><span>Stundenplan</span><select value={selectedElement?.id||""} onChange={event=>{setSelectedElement({type:selectedElementType,id:Number(event.target.value)});setSelected(null)}}>{elementsForSelectedType.map(element=><option key={element.id} value={element.id}>{timetableElementLabel(element)}</option>)}</select></label></div>}
 {(mode==="today"||mode==="week"||mode==="day")&&!previewData&&elementsError&&<p className="plan-picker-message error">{elementsError}</p>}
 {(mode==="today"||mode==="week"||mode==="day")&&!previewData&&!elementsBusy&&!elementsError&&!timetableElements.length&&<p className="plan-picker-message muted">Für dieses Konto wurden keine auswählbaren Stundenpläne freigegeben.</p>}
 {previewData&&<div className="preview-notice"><b>Lokale Vorschau</b><span>Beispieldaten aus der gültigen WebUntis-Antwort · keine echte Abfrage</span></div>}
   {(mode==="week"||mode==="day")&&error&&!displayBlocked&&<div className="notice error" role="alert">{error}<button onClick={()=>load()}>Erneut versuchen</button></div>} {mode==="messages"?<MessagesInbox messages={inboxMessages} busy={inboxBusy} error={inboxError} sourceUrl={inboxSourceUrl} retry={()=>loadInbox()}/>:mode==="homeworks"?<HomeworksView homeworks={filteredHomeworks} busy={homeworksBusy} error={homeworksError} sourceUrl={homeworksSourceUrl} today={previewData?fromUntisDate(previewData.range.startDate):new Date(clock)} courseFilterActive={courseFilter.filterEnabled} retry={()=>loadHomeworks()}/>:<div className={`timetable-output timetable-output-${mode}`}>{mode==="today"?<TodayOverview lessons={todayLessons} date={overviewDate} holidays={previewData||currentWeek?data?.holidays||[]:[]} bounds={timelineBounds} now={new Date(clock)} onSelect={setSelected} messages={messages} messagesBusy={messagesBusy} messagesError={messagesError} messagesSourceUrl={messagesSourceUrl} retryMessages={()=>loadMessages()} timetableBusy={busy||elementsBusy&&!data} timetableMessage={todayTimetableMessage}/>:busy||elementsBusy&&!data?<div className="loader">Stundenplan wird geladen …</div>:displayBlocked?<div className="empty" role="alert"><h2>Anzeige gesperrt</h2><p>Die Schule hat den Stundenplan für diesen Zeitraum in WebUntis nicht freigegeben.</p></div>:error&&!data?null:!previewData&&!selectedElement?<div className="empty">Kein Stundenplan ausgewählt.</div>:mode==="week"?<WeekView week={week} lessonsByDay={lessonsByDay} holidays={data?.holidays||[]} bounds={timelineBounds} now={new Date(clock)} onSelect={setSelected}/>:<><div className="day-tabs" role="tablist" aria-label="Wochentag auswählen">{weekDays.map((label,i)=><button key={label} role="tab" aria-selected={day===i} aria-label={label} className={day===i?"active":""} onClick={()=>setDay(i)}>{label.slice(0,2)}</button>)}</div><div className="day-view"><DayColumn label={weekDays[day]} date={addDays(week,day)} lessons={lessonsByDay[day]} holidays={data?.holidays||[]} bounds={timelineBounds} now={new Date(clock)} onSelect={setSelected}/></div></>}</div>}
