...
import type { Course, Lesson, MessageOfDay, MessagesOfDayPayload, TimetableElement, TimetableElementSelection, TimetableElementType, TimetablePayload } from "@/lib/types";
...
export function Dashboard({displayName,filterStorageId,initialWeek,previewData,previewMessages=emptyMessages,defaultElement}:{displayName:string;filterStorageId:string;initialWeek?:string;previewData?:TimetablePayload;previewMessages?:MessageOfDay[];defaultElement?:TimetableElementSelection|null}) {
 ...
 useEffect(()=>{if(previewData)return;const refresh=()=>{if(document.visibilityState==="visible"&&navigator.onLine){load();if(mode==="today")loadMessages()}};const timer=window.setInterval(refresh,300000);document.addEventListener("visibilitychange",refresh);return()=>{window.clearInterval(timer);document.removeEventListener("visibilitychange",refresh)}},[load,loadMessages,mode,previewData]);
 ...
 function refreshDashboard(){void load();if(mode==="today")void loadMessages()}
