...
export class TimetableDisplayBlockedError extends Error {
  constructor() {
    super("Anzeige gesperrt");
    this.name = "TimetableDisplayBlockedError";
  }
}
...
export async function fetchTimetable(input: LoginInput, person: { personId: number; personType: number }, startDate: number, endDate: number) {
  ...
  try {
    ...
    const shouldLoadSchoolData = !schoolYears.length || Boolean(schoolYear);
    // Check timetable access before loading optional data. A denied date must
    // remain a restriction, never an empty timetable or an offline fallback.
    const lessons = shouldLoadSchoolData
      ? await client.timetable(person.personId, person.personType, startDate, endDate).catch(error => {
          if (error instanceof WebUntisRpcError && error.code === -7004) {
            throw new TimetableDisplayBlockedError();
          }
          throw error;
        })
      : [];
