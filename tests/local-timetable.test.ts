...
test("local timetable selections survive serialization", () => {
  assert.deepEqual(parseTimetableSelection(serializeTimetableSelection({ id: 1781, type: 1 })), { id: 1781, type: 1 });
});
...
test("invalid local timetable selections are rejected", () => {
  ...
  assert.equal(normalizeTimetableSelection({ id: 1781, type: 13 }), null);
});
